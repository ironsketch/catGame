...
poops = multi.Poops(pygame.image.load("src/poo.png"), -30, -30, -7, False)
hairballs = multi.HairBalls(pygame.image.load("src/hairball.png"), -30, -30, -7, False)
colItems = []
...
while True:
    ...
    if(key[K_e] or key[K_p]):
        if(timeNow - player.poopTimer >= poops.poopWait):
            if player.flipped:
                poops.add(pygame.image.load("src/poo.png"), player.rect[0].midright[0], player.rect[0].y - 20, 7, True)
            else:
                poops.add(pygame.image.load("src/poo.png"), player.rect[0].midleft[0] - 10, player.rect[0].y - 20, -7, False)
    ...
    if(key[K_r] or key[K_o]):
        if(timeNow - player.hairBallTimer >= hairballs.hairBallWait):
            if not player.flipped:
                hairballs.add(pygame.image.load("src/hairball.png"), player.rect[0].midright[0], player.rect[0].y - 20, 50, True)
            else:
                hairballs.add(pygame.image.load("src/hairball.png"), player.rect[0].midleft[0] - 10, player.rect[0].y - 20, -50, False)
            player.hairBallTimer = pygame.time.get_ticks()
    
    clouds.move(surf.getWidth())

    # gravity #
    poops.gravity(colItems, surf.getHeight())
    hairballs.gravity(colItems, surf.getHeight())
